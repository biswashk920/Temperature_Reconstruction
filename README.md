...
| Station | Role |
|---|---|
| Tungling | Trained + tested (80/20 split) — paired station and ERA5-Land data used to fit the model |
| Zhang | Trained + tested (80/20 split) — paired station and ERA5-Land data used to fit the model |
| Yalbang | Out-of-domain validation — withheld from training, used to check generalization |
| Halji | Out-of-domain validation — withheld from training, used to check generalization |
| Til | Reconstructed — no station record used; the trained model is applied to Til's long-term (1950–2024) ERA5-Land data to produce the bias-corrected reconstruction and trend analysis |
